if ($env:COVERAGE -eq 1) {
    ...
    (Get-Content -path .coverage_templates\.coveragerc.template -Raw) -replace '`$PWD','$(Get-Location)' | Set-Content -Path "$($env:COVERAGE_PROCESS_START)";
    (Get-Content -path .coverage_templates\sitecustomize.py.template -Raw) -replace '`$PWD','$(Get-Location)' | Set-Content -Path "$($env:PYSITEDIR)/sitecustomize.py";
